...
public abstract class LayerDataGenerator implements DataGenerator {
    ...
    protected NiceDataGenerator nice;
    MyMaze m;
    int t[][];
    ...
    public void generateChunk(final int xStart, final int zStart, final int xEnd, final int zEnd, int spacing,
        ...
        for (int x = xStart; x < xEnd; x++) {
            for (int z = zStart; z < zEnd; z++) {
            	if(x>-1&&x<100&&z>-1&&z<100&&m.grid[x][z]==1){
                generateColumn(x, z,22, blockScene);}
            	else
            	{
            		generateColumnNice(x, z, height, blockScene, xStart, zStart, xEnd, zEnd, treePositions, treeHeights, rand);
            	}
            }
        }
    ...
    private void generateColumnNice(final int x, final int z, final int height, final WorldModifier blockScene,
        ...
        if (localHeight < waterHeight) {
            for (; localHeight < waterHeight; localHeight++) {
                blockScene.setBlock(x, localHeight, z, 0);
            }
        }

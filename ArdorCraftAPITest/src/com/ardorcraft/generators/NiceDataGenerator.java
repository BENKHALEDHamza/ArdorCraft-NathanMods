...
public class NiceDataGenerator implements DataGenerator {
        ...
        for (int x = xStart; x < xEnd; x += spacing) {
            for (int z = zStart; z < zEnd; z += spacing) {
            	if(x>-1&&x<100&&z>-1&&z<100){
            		if(m.grid[x][z]==1){
            		generateColumnCube(x, z, 12, blockScene);
            		}
            	}
            	else{
    ...
    private void generateColumnCube(final int x, final int z, final int height, final WorldModifier blockScene) {
        int startHeight = 1;
